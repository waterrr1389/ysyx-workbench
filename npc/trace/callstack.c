...
void stack_push(word_t pc, word_t target, const char *name) {
  // 1. 分配内存 (使用 malloc)
  CallStackNode *new_node = (CallStackNode *)malloc(sizeof(CallStackNode));
  ...
  new_node->func_name[FUNC_LENGH - 1] = '\0';
...
// 辅助函数：计算当前栈深度，用于打印缩进
int get_stack_depth(void) { return depth; }

// 辅助函数:获取当前栈顶指针
CallStackNode *get_stack_top(void) { return stack_top; }
