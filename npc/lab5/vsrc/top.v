module top(
    ...
    wire [7:0] d2 = ascii; 
    wire [7:0] d1 = buffer[0];
    
    wire seg_en = !release_detected;
    assign dot = 6'b1;

    hex7seg seg0_inst(.en(seg_en), .hex_in(d1[3:0]), .seg_out(seg0));
    hex7seg seg1_inst(.en(seg_en), .hex_in(d1[7:4]), .seg_out(seg1));
    hex7seg seg2_inst(.en(seg_en), .hex_in(d1[3:0]), .seg_out(seg2));
    hex7seg seg3_inst(.en(seg_en), .hex_in(d2[7:4]), .seg_out(seg3));
